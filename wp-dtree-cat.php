...
//		'parent' => $parent
	...
	$unions = array();	
	$query = "(SELECT {$wpdb->posts}.ID AS 'ID', {$wpdb->posts}.post_author as 'post_author', {$wpdb->posts}.post_title AS 'post_title', {$wpdb->terms}.term_id AS 'catid' 
	...
	$print_author_link = strlen($pre_author) > 0;
	$author_name = '';
	$author_url = '';
	foreach($postresults as $postresult){
		if($print_author_link){
